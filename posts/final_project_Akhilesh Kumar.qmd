...
- Graphs (Ggplot2) & Tabular Presentation (Kable) for Year wise/State wise Performance on Crime Chargesheet/Accusation Rate for different category of crimes:
...
- Time Series Plot for State wise, Murder Related Charge-sheet Rate (Yr 1997-2000)

```{r}

plot <- ggplot(crime_murder_yr_state, aes(y = chargesheet_rate, color = as.factor(States_UTs), x = Year)) + 
  geom_line() + 
  geom_point()+
  # facet_wrap( ~ category, nrow = 6 )
  labs(title = "Year wise, State wise, Murder Related Chargesheet Rate")+
    theme_classic()
...
- Time Series Plot for State wise, Kidnap Related Charge-sheet Rate (Yr 1997-2000)

```{r}

plot <- ggplot(crime_kidnap_yr_state, aes(y = chargesheet_rate, color = as.factor(States_UTs), x = as.numeric(as.character(Year)))) + 
  geom_line() + 
  geom_point()+
  # facet_wrap( ~ category, nrow = 6 )
  labs(title = "Time Series Plot for State wise, Kidnap Related Charge-sheet Rate (Yr 1997-2000)")+
    theme_classic()
 ggplotly(plot)
```
...
Time Series Plot for State wise, Crime Against Women Related Charge-sheet Rate (Yr 1997-2000)

```{r}

plot <- ggplot(crime_against_women_yr_state, aes(y = chargesheet_rate, color = as.factor(States_UTs), x = as.numeric(as.character(Year)))) + 
  geom_line() + 
  geom_point()+
  geom_smooth()
  # facet_wrap( ~ category, nrow = 6 )
  labs(title = "Time Series Plot for State wise, Crime Against Women Related Charge-sheet Rate (Yr 1997-2000)")+
    theme_classic()
