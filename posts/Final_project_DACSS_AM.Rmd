...
```{r}
crime_year_dist_B2 <- crime_year_dist_B2 %>% 
  select(assault = `Assault/Battery`, `Investigate Person/Property/Other`, Larceny, `Motor Vehicle`, Property)

```

```{r}

plot_ly(crime_year_dist_B2, x = ~YEAR, y = ~assault, name = "Assault / Battery",
         type = 'scatter', mode = 'line') %>%
  add_trace(y = ~`Investigate Person/Property/Other`, name = 'Investigate Person/Property', mode = 'line') %>% 
...
In recent years, the number of motor vehicle crime incidents in the Roxbury District has fluctuated. In 2017, there were 1782 reported incidents. This number declined to 1515 in 2020, but then increased to 1858 in 2021. While the number of incidents did decline to 1536 in 2022, it is still higher compared to 2020 and cannot be considered a declining trend at this point.
...
In recent years, the number of motor vehicle crime incidents in the Dorchester District has fluctuated. In 2017, there were 1436 reported incidents. This number declined to 1302 in 2019, but then increased to 1793 in 2021. While the number of incidents did decline to 1468 in 2022, it is still higher compared to 2020 and cannot be considered a declining trend at this point. This suggests that motor vehicle crimes remain a significant concern in the Dorchester District.

On the other hand, assault & battery related crimes have been declining in the Dorchester District from 2018 to 2020. However, there was an increase in assault & battery crime incidents from 503 in 2020 to 530 in 2021, which could be a matter of concern for the Boston Police Department. It's worth noting that larceny cases have been consistently declining in the Dorchester District since 2019, which could be considered a declining trend.
...
Motor vehicle crime incidents have also been rising in the South End District, with 935 cases in 2021 and 949 cases in 2022. This increase could be a matter of concern for the Boston Police Department.

On the other hand, assault & battery crime incidents have been declining in the South End District from 2019, with 985 cases in 2019 and 581 cases in 2022. However, the trend has remained stable between 2021 and 2022, with 581 assault & battery cases in both years.
...
Motor vehicle crime incidents, have also increased from 302 in 2019 to 767 in 2022. However, there was a decrease to 287 in 2020.

Assault and battery crime incidents, which involve the use of physical force or violence against another person, is almost at the same level  with 307 in 2019 to 283 in 2022. However, there was a decrease to 235 in 2020.
...
leaflet() %>%
  ...
  addCircleMarkers(data = dtmap2,
                   ...
                   label = ~as.character(STREET),
                   clusterOptions = markerClusterOptions())%>%
addHeatmap(data = dtmap2,lng=~Long, lat=~Lat, blur=15, max =10, radius = 9)
```
...
dtmap3 <-  crime %>%
  select(STREET, Lat, Long, OFFENSE_DESCRIPTION, YEAR, MONTH) %>% 
  filter(!is.na(STREET)) %>%
  filter(!is.na(Lat)) %>% 
  filter(!(Lat %in% c(-1,1))) %>% 
  filter(!is.na(Long)) %>% 
  filter(!(Long %in% c(-1,1))) %>% 
  filter(YEAR == sort(unique(YEAR), decreasing = TRUE)[1]) %>% 
  filter(MONTH %in% c(c(sort(unique(MONTH), decreasing = TRUE)[1:2]))) %>% 
  group_by(STREET, OFFENSE_DESCRIPTION) %>%
  summarise(frequency = n()) %>% 
  slice_max(order_by = frequency, n=2) %>% 
  mutate(temp = paste0(OFFENSE_DESCRIPTION, " (",frequency,") " ,collapse = ";"))%>% 
  distinct(STREET, .keep_all = TRUE)


dtmap3 <- left_join(dtmap3, street_coordinates, by = "STREET" ) %>% 
  distinct(STREET, .keep_all = TRUE)


dtmap3$content <- paste("<b>Street:</b>",dtmap3$STREET,"<br/>",
                        "<b>Most Recent Offense:</b>", dtmap3$temp)

colors <- c("green", "orange", "red")
pal <- colorFactor(colors, dtmap3$total)
