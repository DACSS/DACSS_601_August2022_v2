...
The simplest univariate vizualization for this data set seems to be a bar graph. I decided to display the "yes" votes for each territory. I wanted them in numerical order, which is seen in my code, but I couldn't get the information to display the way I wanted.

```{r}
#top 10 territories voting "yes"
yes_vote <- aus_marriage %>% filter(resp == "yes") %>%
  group_by(territory, count) %>%
  summarize(Total = n()) %>%
  arrange(desc(count)) %>% ungroup() %>% head(10)

ggplot(yes_vote, aes(x = territory, y = count))+
