---
title: "Final Project (Work-in-Progress II)"
...
::: callout-warning
to do: if-else for package installing, in text citations, kable() specifications
:::
...
In 2020, Snapchat had around 249 million active users on its platform (Rodriguez, 2020).
...
Which candidate's advertisements reached more people? How much was spent on these advertisements? Which states were targeted by these advertisements?
...
missing_val_plot <- missing.values %>%
      ggplot() +
        geom_bar(aes(x = reorder(key, desc(percents)), 
                 ...
                 stat = 'identity', alpha=0.8) + scale_fill_manual(name = "", values = c('pink', 'grey'), labels = c("Present", "Missing")) +
      coord_flip() + theme(axis.text.y = element_text(size = 5)) + labs(title = "Percentage of Missing Values", x = 'Variable', y = "Missing values %")
...
### Highest expenditure and impressions on a single ad by candidate
...
```
The highest number of impressions for ads supporting Biden was 17,927,667, while for Trump it was 31,848,256. The higher number of impressions for Trump's ad could be attributed to it not having a set end date.

### Relative ad expenditure and impressions over time by candidate

I'm interested in knowing the relative expenditure and impressions for advertisements by candidate. First, I want to extract the month from the `start_date` and `end_date` columns and use it to determine spending over the months.

```{r}
#| label: new columns for the months that the advertisements began and stopped running on Snapchat
...
I'll need to take a log transformation because the values in the `spend` column are skewed. I'm using a box plot since I want to compare the distribution of `spend` for the two values in `candidate_ballot_information`.
...
On average, ads for Biden spent more that ads for Trump. Also, political ads supporting Biden's campaign were more frequent in the months leading up to election day, i.e. July to November.

It would be worthwhile to compare the impressions of advertisements for both candidates too.
...
Advertisements supporting Trump's campaign seem to have reached more people than Biden's advertisements. However, as noted before, impressions reached for advertisements for Biden's campaign were more prominent in the later months of the year.
...
#### Creating a table for location analysis

Checking the distribution of values in the `spend` and `impressions` columns:

```{r}

ggplot(polads, aes(x=spend)) + geom_histogram()

ggplot(polads, aes(x=impressions)) + geom_histogram()

```

Clearly, both distributions are skewed to the right and are not symmetric. Hence, I'm taking the median of these columns for analysis.
...
#### Visualizing ad expenditure across states

```{r}
# plotting expenditure
loc_inc_spend_plot <- plot_usmap(data = polads_loc1, values = "spend_median", labels = FALSE) + scale_fill_viridis_c(name = "Median Ad Expenditure") +
...
loc_inc_spend_biden_plot <- plot_usmap(data = polads_loc1_biden, values = "spend_median", labels = FALSE) + scale_fill_viridis_c(name = "Median Ad Expenditure") +
...
loc_inc_spend_trump_plot <- plot_usmap(data = polads_loc1_trump, values = "spend_median", labels = FALSE) + scale_fill_viridis_c(name = "Median Ad Expenditure") +
...
# comparing plots
loc_inc_spend_biden_plot
loc_inc_spend_trump_plot

```

Biden: Pennsylvania, Nebraska - seems that biden had more ads that targetted no states in particular Trump: Texas, Mississippi, South Carolina - explicitly targeted all states

-   expenditure over the months
```{r}
#| eval: false
...
# plotting expenditure
loc_inc_spend_month_plot <- plot_usmap(data = polads_loc_month1, values = "spend_median", labels = FALSE,label_color = "black") + scale_fill_viridis_c(name = "Ad Expenditure Amount by Month") +
labs(title = "Snapchat Targeted Political Ad Expenditure Across the States") + theme(legend.position = "right")

loc_inc_spend_month_plot

#animating change in median expenditure by month
loc_inc_spend_month_transition <- loc_inc_spend_month_plot +
  labs(title = "Total Political Ad Expenditure in {as.numeric(frame_time)}") + transition_time(as.numeric(start_month))

loc_inc_spend_anim <- animate(loc_inc_spend_month_transition, fps=10) + ease_aes('linear')
loc_inc_spend_anim

```

#### Visualizing ad impressions across states

```{r}

# plotting impressions
loc_inc_imp_plot <- plot_usmap(data = polads_loc1, values = "impressions_median", labels = FALSE) + scale_fill_viridis_c(name = "Median Ad Impressions") +
...
Mississippi and South Carolina had the highest median ad impressions.
...
loc_inc_imp_biden_plot <- plot_usmap(data = polads_loc1_biden, values = "impressions_median", labels = FALSE) + scale_fill_viridis_c(name = "Median Ad Impressions") +
...
loc_inc_imp_trump_plot <- plot_usmap(data = polads_loc1_trump, values = "impressions_median", labels = FALSE) + scale_fill_viridis_c(name = "Median Ad Impressions") +
...
# comparing plots
loc_inc_imp_biden_plot
loc_inc_imp_trump_plot

```

-   Biden: Nebraska, Pennsylvania
-   Trump: Texas, Mississippi, South Carolina
...
-   the potential of political advertisements on social media - more impressions, more impact
...
-   targeting states that were predominantly Democratic or Republican
-   spent a lot of time on cleaning the data to prepare it for analysis
-   I learnt a lot while making this project - new packages like 'patchwork' and 'janitor', more familiarized with the tidyverse

## Further Directions and Conclusion

-   Using the postal codes and latitude-longitude columns to map more specific regions
-   add table with state populations and check spend and impressions verse population
-   type of agency paying for ad - own campaign or outside?
-   impressions sum - by organization
-   spend sum - by organization

## References

::: {#refs}

California Fair Political Practices Commission. (2020). *Electronic Media Advertisements* [PDF]. Retrieved 3 September 2022, from [https://www.fppc.ca.gov/content/dam/fppc/NS-Documents/AgendaDocuments/Task-Force/dttf-2020/march-2020/Legal.pdf](https://www.fppc.ca.gov/content/dam/fppc/NS-Documents/AgendaDocuments/Task-Force/dttf-2020/march-2020/Legal.pdf).

*Campaign Disclosure, Filer Resources, Alaska Public Offices Commission, Department of Administration, State of Alaska*. Alaska Department of Administration. Retrieved 3 September 2022, from [https://doa.alaska.gov/apoc/FilerResources/campaignDisclosure.html](https://doa.alaska.gov/apoc/FilerResources/campaignDisclosure.html).

*Contribution Limits*. Campaign Spending Commission. Retrieved 3 September 2022, from [https://ags.hawaii.gov/campaign/contribution-limits/](https://ags.hawaii.gov/campaign/contribution-limits/).
...
Rodriguez, S. (2020). *Snap stock rockets up after surprise earnings beat*. CNBC. Retrieved 3 September 2022, from [https://www.cnbc.com/2020/10/20/snap-earnings-q3-2020.html](https://www.cnbc.com/2020/10/20/snap-earnings-q3-2020.html).
